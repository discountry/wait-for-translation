...
你也可以在你的媒体查询中运用基本的逻辑，这允许你结合很多种情况。例如，你可以使用以下的逻辑：
```
...
```
你也可以结合`AND`和`OR`逻辑来创建更复杂的查询。下面这个页面当前在一个很大尺寸的手机上渲染，但是是在横屏模式：
...
### `Only`和`Not`的逻辑查询
我们也可以用`only`和`not`来创建精准的媒体查询。例如，下面这种情况将会匹配至少640px宽度的设备但是除了打印媒体类型：
```
